...
- **[S3 enforcement]** Apply an `aws:SecureTransport` condition and require server-side encryption on the S3 buckets Athena reads from and writes to, enforced via bucket policy or the `s3-bucket-ssl-requests-only` AWS Config rule. [doc](https://docs.aws.amazon.com/athena/latest/ug/internetwork-traffic-privacy.html)
